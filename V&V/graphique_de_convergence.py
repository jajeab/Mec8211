...
def generate_fake_data(char):
    ...
    #error_values *= np.exp(np.random.normal(scale=0.075, size=num_points))
...
def graphes(char):
    ...
    coefficients = np.polyfit(np.log(h_values[:3]), np.log(error_values_1[:3]), 1)
    ...
    plt.scatter(h_values, error_values_1, marker='o', color='b', label='Données numériques obtenues')
    ...
    plt.title(f'Convergence d\'ordre {exponent:.4f}\n de l\'erreur $L_1$ en fonction de $Δx$',
    ...
    # Tracer le graphique en échelle log-log avec des points et la courbe de régression extrapolée
    #plt.figure(figsize=(8, 6))
    plt.scatter(h_values, error_values_2, marker='o', color='b', label='Données numériques obtenues')
    plt.plot(h_values, fit_function(h_values), linestyle='--', color='r', label='Régression en loi de puissance')
    ...
    # Ajuster une loi de puissance à toutes les valeurs (en utilisant np.polyfit avec logarithmes)
    coefficients = np.polyfit(np.log(h_values[:3]), np.log(error_values_inf[:3]), 1)
    exponent = coefficients[0]
    ...
    # Tracer le graphique en échelle log-log avec des points et la courbe de régression extrapolée
    #plt.figure(figsize=(8, 6))
    plt.scatter(h_values, error_values_inf, marker='o', color='b', label='Données numériques obtenues')
    plt.plot(h_values, fit_function(h_values), linestyle='--', color='r', label='Régression en loi de puissance')
    ...
    # Ajouter des étiquettes et un titre au graphique
    plt.title(f'Convergence d\'ordre {exponent:.4f}\n de l\'erreur $L_∞ $ en fonction de $Δx$',
            fontsize=14, fontweight='bold', y=1.02)  # Le paramètre y règle la position verticale du titre

    plt.xlabel('Taille de maille $h_{max}$ ou $Δx$ (cm)', fontsize=12, fontweight='bold')  # Remplacer "h" par "Δx"
    plt.ylabel('Erreur $L_∞ $ (m/s)', fontsize=12, fontweight='bold')
